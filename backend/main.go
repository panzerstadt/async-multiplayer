...
func main() {
	...
	savesGroup.Use(rateLimitMiddleware(10, time.Minute)) // 10 requests per minute
	savesGroup.POST("", uploadSaveHandler(db))
	savesGroup.GET("/latest", getLatestSaveHandler(db))
...
func joinGameHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		player := Player{
			UserID:    userUUID,
			GameID:    gameID,
			TurnOrder: 0, // This should be calculated based on existing players
		}
